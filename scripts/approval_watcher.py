...
from config import CREDENTIALS_PATH, TOKEN_PATH, APPROVED, DONE, FAILED, LOGS, ENV_PATH
...
from workflow_utils import append_log, extract_email_address, read_markdown_with_frontmatter

# Add the scripts directory to the path so we can import other modules when
# launched by macOS services with a different working directory.
...
class ApprovalWatcher(FileSystemEventHandler):
    def __init__(self, approved_dir, done_dir, failed_dir):
        ...
        self.receipts_path = LOGS / "execution_receipts.json"
        self.executed_action_ids = self.load_execution_receipts()
    ...
    def load_execution_receipts(self):
        """Load executed action IDs to prevent duplicate sends/posts."""
        if not self.receipts_path.exists():
            return set()
        try:
            data = json.loads(self.receipts_path.read_text(encoding='utf-8'))
            if isinstance(data, list):
                return {item.get('action_id') for item in data if item.get('action_id')}
        except (json.JSONDecodeError, OSError):
            pass
        return set()

    def save_execution_receipt(self, action_id, action_type, filename, details=None):
        if not action_id:
            return
        ...
        if self.receipts_path.exists():
            try:
                parsed = json.loads(self.receipts_path.read_text(encoding='utf-8') or '[]')
        ...
        self.receipts_path.write_text(json.dumps(entries, indent=2), encoding='utf-8')
    ...
    def process_file(self, file_path):
        ...
        try:
            ...
            action_type = str(frontmatter.get('type', '')).lower()
            action_id = frontmatter.get('action_id') or f"{action_type}:{file_path.name}"
            if action_id in self.executed_action_ids:
                self.logger.warning(f"Action already executed, moving duplicate to Done/: {action_id}")
                append_log('duplicate_execution_skipped', file_path.name, {'action_id': action_id})
            ...
            if action_type == 'linkedin_post':
                if process_markdown_file:
                    ...
                    if not success:
                        ...
                        self.save_execution_receipt(action_id, action_type, file_path.name)
                else:
                ...
                if not to:
                    to = extract_email_address(frontmatter.get('from', ''))
                ...
                body = frontmatter.get('draft_body')
                if not body:
                    self.logger.error(
                        f"Approved email {file_path} has no draft_body; refusing to generate after approval"
                    )
                    append_log('email_missing_approved_body', file_path.name, {'action_id': action_id})
                    self.move_to_failed(file_path)
                    return
                ...
                try:
                    ...
                    self.save_execution_receipt(
                        action_id,
                        action_type,
                        file_path.name,
                        {'to': to, 'subject': subject}
                    )
                    append_log('email_sent', file_path.name, {'action_id': action_id, 'to': to, 'subject': subject})
                    self.move_to_done(file_path)
                except Exception as e:
